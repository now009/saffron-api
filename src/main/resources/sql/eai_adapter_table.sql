핵심 포인트 요약
DB Adapter — DataSource 연결 정보(eai_datasource)와 인터페이스별 실행 설정(eai_db_adapter_config)을 2개 테이블로 분리했습니다.
           하나의 DataSource(DS_ERP)에 여러 인터페이스가 매핑될 수 있기 때문입니다. Statement ID는 MyBatis Mapper 경로로 관리합니다.
REST Adapter — eai_rest_config 단일 테이블로 구성됩니다. 인증 방식이 5가지(NONE / BEARER / API_KEY / BASIC / OAUTH2)라
             OAUTH2 전용 컬럼(token_url, client_id, client_secret)을 함께 포함했습니다.
SOAP Adapter — eai_soap_config에 WSDL 정보, WS-Security 인증, SOAP Action, MTOM 설정을 모두 담습니다.
             WSDL은 매번 파싱하면 느리므로 wsdl_cache 컬럼에 캐싱합니다.
FILE Adapter — eai_file_config에서 가장 컬럼이 많습니다. done_path (처리 완료 이동 경로)가 핵심인데,
             이게 없으면 재시작 시 같은 파일을 중복 처리하는 문제가 발생합니다.
공통 원칙 — 비밀번호·토큰·키는 전부 ENCRYPTED:{base64_AES256} 형식으로 저장하고,
               런타임에 AesEncryptor 빈이 복호화합니다.
...
drop table eai_datasource;
drop table eai_db_adapter_config;
drop table eai_rest_config;
drop table eai_soap_config;
