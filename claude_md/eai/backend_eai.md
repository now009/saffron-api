...
- 이에 따른 DTO를 만들고 controller, service를 만들어줘
- 기본적인 crud 기능을 만들고 end point를 알려줘
  Push 완료. 엔드포인트 정리입니다.
...
각 화면에서 백엔드로 보내는 페이로드를 정리하면 다음과 같습니다. (모든 요청 공통: Content-Type: application/json, 인증헤더 동봉)

}

위의 페이로드와 DB컬럼을 기준으로 기존코드를 수정하거나 추가해주고 endpoint를 정리해줘